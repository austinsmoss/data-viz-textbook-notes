...
Everything in R is an *object* that has a *name*, including variables (like `x` or `y`), datasets (like `mydata`), and functions (like `mean()`). This code creates an object named `mynumbers` that is a vector of numbers.
